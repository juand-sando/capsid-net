...
def load_node_colors(node_colors_file):
    """Read CSV with columns: group,color  →  dict {group: color}."""
...
def get_center_and_normal(point_dict):
    """
    Accepts a dictionary with exactly 3 keys, each mapping to a 3D NumPy array.
    Returns a NumPy array: [rx, ry, rz, nx, ny, nz]
    """
...
def build_group_dictionaries(capsomer_df, grouping_df):
    ...
    for _, row in capsomer_df.iterrows():
        ...
        suffix = chain_name[len(base_name):]  # keeps '_x1', '_x2', etc.
...
def calculate_distances(df):
    """
    Given a DataFrame with rx, ry, rz columns, return a ZxZ Euclidean distance matrix.
    """
...
def calculate_angles(df, relatedness_file):
    """
    Given a DataFrame with nx, ny, nz and a relatedness CSV file,
    return a KxK matrix of angles (in degrees), where interactions are defined by relatedness < 1.
    If angle > 90, tries flipping one normal; if still > 90, raise error.
    """
    relatedness_df = pd.read_csv(relatedness_file, index_col=0)
    group_names = df["group"].values

    # Ensure all needed groups are in the relatedness matrix
    ...
    for i in range(num):
        for j in range(i + 1, num):
            if interaction_mask[i, j]:
                ...
                if angle > 90:
                    # Try flipping one of the vectors
                    ...
                    if angle_flipped <= 90:
                        ...
                        raise ValueError(f"Cannot resolve orientation between vectors {i} and {j} (angle={angle:.2f}°, flipped={angle_flipped:.2f}°)")

                angle_matrix[i, j] = angle
                angle_matrix[j, i] = angle  # Symmetric
...
def plot_angle_network(center_normal_df, distance_matrix, angle_matrix, output_dir, title="Intercapsomer angle", node_colors_map=None):
    ...
    for label in group_labels:
        if "_" in label:
            ...
            display_label[label] = base + "'"   # A_x1 -> A'
    ...
    # Add edges with angle-based color and thickness
    angles = angle_matrix
    real_angles = angles[angles>0.0]
    edges = []
    edge_colors = []
    edge_widths = []
    ...
    lower_limit = 1
    upper_limit = 30
    cmap = colormaps['YlOrRd']  # Red = large angle, Green = small
    ...
    node_colors = []
    if node_colors_map:
        # Use provided mapping; fall back to lightgrey when missing
        for label in group_labels:
        ...
        min_frac = 0.40
        max_frac = 0.90
        # Extract subscript numbers
        subscript_numbers = [int(re.search(r"_x(\d+)", label).group(1))
            for label in group_labels if "_x" in label]
    ...
    nx.draw_networkx_nodes(G, pos_dict, node_color=node_colors, node_size=500, edgecolors="black", linewidths= 0.8,  ax=ax)
    # Build label dict with subscript formatting
    label_dict = {label: display_label[label] for label in group_labels}
    for i, label in enumerate(group_labels):
        if had_subscript[label]:
            node_colors[i] = "white"


    nx.draw_networkx_labels(G, pos_dict, labels=black_labels, font_size=16, font_weight='bold', ax=ax, font_family="Arial")
    nx.draw_networkx_labels(G, pos_dict, labels=white_labels, font_size=16, font_weight='bold', ax=ax, font_family="Arial", font_color="white")
    nx.draw_networkx_edges(G, pos_dict, edgelist=edges, edge_color=edge_colors, width=edge_widths, ax=ax)

    # Colorbar
    sm = plt.cm.ScalarMappable(cmap=cmap, norm=norm)
    sm.set_array([])
    cbar = plt.colorbar(sm, ax=ax, shrink=0.5, orientation='horizontal',extend="both")
    cbar.set_label("Inter-capsomer angle, φ (°)", fontsize=13, fontweight="bold", labelpad = 5)

    # Make tick lines and labels more visible
...
def main():
    parser = argparse.ArgumentParser(description="Capsomer normal & distance analysis.")
    ...
    parser.add_argument('--custom_filter', help='Optional .txt file with chain names to include in the network')
    parser.add_argument('--output', default='results', help='Output directory for CSVs and plots')
    parser.add_argument('--node_colors', help='Optional CSV mapping group -> color for node coloring')
    ...
    # Build groupings & compute center/normal
    ...
    center_normal_df = compute_centers_and_normals(groupings_dict)
     ...
     # Optional filtering by --custom_filter
    if args.custom_filter:
        with open(args.custom_filter, 'r') as f:
            allowed_groups = set(line.strip() for line in f if line.strip())
